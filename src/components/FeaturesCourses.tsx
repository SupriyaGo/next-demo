...
export default function FeaturesCourses() {
    const featuredCourses = musicData.courses.filter((course: Course) => course.isFeatured === true);
  return (
                            ...
                            <div key={course.id} className="flex justify-center">
                                <BackgroundGradient className="flex flex-col rounded-[20px] bg-white dark:bg-zinc-900 overflow-hidden h-full max-w-sm">
                                    <div className="p-4 sm:p-6 flex flex-col items-center text-center flex-grow">
                                        <Image src={course.image} alt={course.title} width={100} height={100} className='w-full rounded-lg' /> 
